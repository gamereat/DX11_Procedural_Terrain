...

{
	...
	verticalTexture = new RenderTexture(directX3D->GetDevice(), ApplicationSettings::screenWidth / downscaleValue, ApplicationSettings::screenHeight / downscaleValue, ApplicationSettings::sceenNear, ApplicationSettings::screenDepth);
	
	hoizontalTexture = new RenderTexture(directX3D->GetDevice(), ApplicationSettings::screenWidth / downscaleValue, ApplicationSettings::screenHeight / downscaleValue, ApplicationSettings::sceenNear, ApplicationSettings::screenDepth);
...
RenderTexture * PostProcessingGaussianBlur::RenderHoizontalBlur(D3D * directX3D, Camera * camera, OrthoMesh * orthoMesh, RenderTexture * currentRenderTexture)
{
	if (isHoizontalEnabled)
	{
		...
		horizontalBlurShader->SetShaderParameters(directX3D->GetDeviceContext(), worldMatrix, baseViewMatrix, orthoMartix, currentRenderTexture->GetShaderResourceView(), ApplicationSettings::screenWidth / downScaleAmmount, neighboursToUse);
...
RenderTexture * PostProcessingGaussianBlur::RenderVerticalBlur(D3D * directX3D, Camera * camera, OrthoMesh * orthoMesh, RenderTexture * currentRenderTexture)
{
	if (isVerticalEnabled)
	{
		...
		VerticallBlurShader->SetShaderParameters(directX3D->GetDeviceContext(), worldMatrix, baseViewMatrix, orthoMartix, currentRenderTexture->GetShaderResourceView(), ApplicationSettings::screenWidth / downScaleAmmount, neighboursToUse);
