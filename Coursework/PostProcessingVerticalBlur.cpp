...
RenderTexture * PostProcessingVerticalBlur::Render(D3D * directX3D, Camera * camera, OrthoMesh * orthoMesh, RenderTexture * currentRenderTexture)
{
	if (isEnabled)
	{
		...
		verticalBlurShader->SetShaderParameters(directX3D->GetDeviceContext(), worldMatrix, baseViewMatrix, orthoMartix, currentRenderTexture->GetShaderResourceView(), ApplicationSettings:: / downScaleAmmount, neighboursToUse);
